...
void main()
{
    ...
    for (int i = 0; i <= instances; i++)
    {
        ...
        vec3 offset = vec3(i / NR_COLS * INSTANCE_OFFSET.x , 0, j *INSTANCE_OFFSET.z);

        j++;
        if(j > NR_COLS) j = 1;

        texture_coord = v_texture_coord[0];
        EmitPoint(p1 / shrinkFactor, offset);

        texture_coord = v_texture_coord[1];
        EmitPoint(p2 / shrinkFactor, offset);

        texture_coord = v_texture_coord[2];
        EmitPoint(p3 / shrinkFactor, offset);
